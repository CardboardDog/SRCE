...
def load_height(file, level, scale):
    ...
    return textureBuffer
def load_roof_pixel(file, level, x, y):
    gameFile = open(file,'r')
    settingsOpen = 0
    scanY = 0
    beginLevelName = "NAME "+level+"\n"
    ...
    for charBuffer in lines:
        ...
        if(settingsOpen==1 and charBuffer==beginLevelName):
            settingsOpen = 2
        if(settingsOpen==2 and charBuffer=="RHGHT:\n"):
            settingsOpen = 3
            scanY = 0
        elif(settingsOpen==3):
            if(charBuffer=="END\n"):
    ...
    gameFile.close()
    return 0
def load_roof(file, level, scale):
    textureBuffer = [[0 for x in range(scale[0])] for y in range(scale[1])] 
    for y in range(scale[1]):
        for x in range(scale[0]):
            textureBuffer[x][y]=load_roof_pixel(file,level,x,y)
    return textureBuffer
...
def read_file_levels(file):
    ...
    for i in fileGroups:
        ...
        tupleSet = (levelInfo,load_height(file,i,(levelInfo[2],levelInfo[3])),load_roof(file,i,(levelInfo[2],levelInfo[3])))
